...
class HashidsHelper
{
    public static function dcryptID($encrypted_id, $connection = 'main')
    {
        try {
            $text = Hashids::decode($encrypted_id);
            dd($text);
            if (count($text) > 0) {
                return $text[0];
            } else {
    ...
    public static function cryptID($id, $connection = 'main')
    {
        try {
    ...
    public static function enCodeId($text)
    {
        if (!isset($text)) {
